/**
 * Unit tests for the new Search API implementation
 */
...
import { initializeEventPublisher, SyncHelpers, publishDatabaseEvent } from '../../src/services/sync';

// Mock external dependencies
jest.mock('typesense');
jest.mock('meilisearch');

describe('New Search API Implementation', () => {
  ...
  describe('Sync System', () => {
    test('should initialize sync components', async () => {
      const publisher = initializeEventPublisher();
      expect(publisher).toBeDefined();
      expect(publisher.getStatus).toBeDefined();
      
      const status = publisher.getStatus();
      expect(status.isHealthy).toBe(true);
      expect(status.totalPublished).toBe(0);
      expect(status.totalErrors).toBe(0);
    });

    test('should create database change events', () => {
      const insertEvent = SyncHelpers.createInsertEvent('claims', { id: 'claim-123', status: 'OPEN' });
      expect(insertEvent.eventType).toBe('INSERT');
      expect(insertEvent.documentType).toBe('claims');
      expect(insertEvent.data.id).toBe('claim-123');
      expect(insertEvent.metadata?.source).toBe('api');
      
      const updateEvent = SyncHelpers.createUpdateEvent('claims', { id: 'claim-123', status: 'CLOSED' }, { id: 'claim-123', status: 'OPEN' });
      expect(updateEvent.eventType).toBe('UPDATE');
      expect(updateEvent.oldData.status).toBe('OPEN');
      expect(updateEvent.data.status).toBe('CLOSED');
      
      const deleteEvent = SyncHelpers.createDeleteEvent('claims', 'claim-123');
      expect(deleteEvent.eventType).toBe('DELETE');
      expect(deleteEvent.data.id).toBe('claim-123');
    });

    test('should handle event publishing', async () => {
      // This will use the no-op publisher since no SNS topic is configured
      const result = await publishDatabaseEvent('INSERT', 'claims', { id: 'test-claim' });
